...
func TestHub_Run(t *testing.T) {
	...
	// Check that if a Wait times out on its own context before the hub exits,
	// we get the expected error.
	t.Run("WaitTimeout", func(t *testing.T) {
		ctx, cancelWait := context.WithTimeout(context.Background(), 50*time.Millisecond)
		defer cancelWait()

		got, want := h.Wait(ctx), hub.ErrHubRunning
		if got != want {
			t.Errorf("Wait with timeout: got %v, want %v", got, want)
		}
	})
	...
	t.Run("WaitMatchesRun", func(t *testing.T) {
		cancelHub()
		werr := h.Wait(context.Background()) // no timeout
		rerr := <-errc
