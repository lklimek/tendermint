...
func (h *Hub) Run(ctx context.Context) error {
	...
	// N.B. We want a background context here, since the run context may have
	// already ended. Because we've closed h.errc, this will not block.
	return h.Wait(context.Background())
}

// Wait blocks until ctx ends or until h exits, and reports the exit status of
// the hub.  If ctx ends before the hub exits, Wait returns ErrHubRunning.
// This can be used to effect a poll of the run state. Otherwise, Wait returns
// the same value reported by Run.
func (h *Hub) Wait(ctx context.Context) error {
	select {
	case <-ctx.Done():
		return ErrHubRunning
		// N.B. Not ctx.Err() so the caller can distinguish between the hub
		// context ending and the Wait request timing out.
