...
func (lst *Listener) accept(incoming Item) {
	if !lst.wantLabel(incoming.Label) {
...
type ListenOptions struct {
	...
	// If set, only listen to items whose label matches this glob pattern.
	// WIthin the pattern, "*" matches a run of arbitrary characters.
	// If empty, all items are delivered regardless of label.
	MatchLabel string
}
...
func (o *ListenOptions) matchLabel() func(string) bool {
	if o == nil || o.MatchLabel == "" {
		return match.Any
	}
	return match.Glob(o.MatchLabel)
}
