...
func TestStore(t *testing.T) {
	...
	t.Run("ScanAll", func(t *testing.T) {
		var numEntries int
		if err := s.Scan(ctx, index.Query{}, func(e index.Entry) error {
			numEntries++
	...
	t.Run("ScanFiltered", func(t *testing.T) {
		const want = "foo"
		var numEntries int
		if err := s.Scan(ctx, index.Query{MatchLabel: match.Exactly(want)}, func(e index.Entry) error {
			if e.Label != want {
	...
	t.Run("ScanPartialRange", func(t *testing.T) {
		var numEntries int
		if err := s.Scan(ctx, index.Query{Start: c2}, func(index.Entry) error {
			numEntries++
			return nil
		}); err != nil {
			t.Errorf("Scan: unexpected error: %v", err)
	...
	t.Run("ScanEmptyRange", func(t *testing.T) {
		if err := s.Scan(ctx, index.Query{Start: "Z", Limit: "A"}, func(e index.Entry) error {
